...
//   verifyOtpActionRegister,
// } from '../../stores/user/user-action';

// import { getDataRegister, getEmail, useLoading } from '../../stores/user/user-store';
// import { ModalTypeEnum } from '../../enums';

// import OTPInput from 'react-otp-input';

// const VerifyOtpRegister = ({ inCartView, loginCallback }: any) => {
...
//   const email: any = getDataRegister();

//   console.log(email?.dataRegister?.email?.email,"sdbnjdskbsdjkbjk")
...
//     // const payload = {
//     //   otp: otp,
//     //   email: email?.dataRegister?.email,
//     // };
//     const payload = {
//         otp: otp,
...
//         newsletter: email?.dataRegister?.email?.newsletter || 0,
//         phone: email?.dataRegister?.email?.phone || '',
//         pincode: email?.dataRegister?.email?.pincode || '',
//         permission: email?.dataRegister?.email?.permission || ''
//       };

//       console.log(payload,"sdbdshj")
//     verifyOtpActionRegister(payload);
//     setIsOtpValid(true);
//   };
//   useEffect(() => {
...
//       email: email?.dataRegister?.email,
//     };
//     loginUsingOtpActionRegister(payload, inCartView);
...
import { getDataRegister, useLoading } from '../../stores/user/user-store';
import { ModalTypeEnum } from '../../enums';
...
const VerifyOtpRegister = ({ inCartView, loginCallback }: any) => {
  ...
  const [otp, setOtp] = useState('');
  const [isResendDisabled, setIsResendDisabled] = useState(false); // State to manage resend button's disabled/enabled status
  const intervalRef = useRef<any>(null);

  const [timer, setTimer] = useState(900); // Timer set to 15 minutes
  const [isOtpValid, setIsOtpValid] = useState(true);
      ...
      >
        ...
        <p className="register text-14">
          ...
          <span
            ...
            }} // Apply inline styles to disable the resend button
          >
            {!isOtpValid && `${t('expired')} `}
            {t('resend')}
          </span>
          <span className="register-text">{`   ${Math.floor(timer / 60)}:${
            timer % 60
          }`}</span>
        </p>
        <Button
